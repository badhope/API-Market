/**
 * apilist.fun importer — fetches from their curated directory.
 *
 * apilist.fun is a beautifully designed directory of public APIs.
 * Less comprehensive than RapidAPI but better curated and easier
 * to browse.
 *
 * Source: https://apilist.fun
 */
...
const SOURCE_URL = "https://apilist.fun"
const CACHE_PATH = join("/workspace/API-market/data/.cache", "apilist-fun.json")

export interface ApilistFunOptions {
  dataDir?: string
  dryRun?: boolean
}

export interface ApilistFunResult {
  apis: number
  categories: number
  lastVerified: string
}

interface ApilistFunEntry {
  name: string
  description: string
  url: string
  category: string
...
export async function fetchApilistFun(): Promise<ApilistFunEntry[]> {
  ...
  console.warn("  ⚠ apilist.fun: No API endpoint found, returning empty list")
  return []
}

/**
 * Convert apilist.fun entry to our internal format.
 */
function toScoredApi(entry: ApilistFunEntry, lastVerified: string): ScoredApi | null {
...
export async function importApilistFun(
  options: ApilistFunOptions = {},
): Promise<ApilistFunResult> {
  const dataDir = options.dataDir ?? "/workspace/API-market/data"
  const lastVerified = new Date().toISOString().slice(0, 10)

  console.log("  Fetching apilist.fun API list...")
  const entries = await fetchApilistFun()

  if (entries.length === 0) {
    console.log("  No APIs found (apilist.fun may not expose a public API)")
    return {
      apis: 0,
      categories: 0,
      lastVerified,
